  <template>
    <div class="card" style="position:flex;top:100px;width: 500px;">
    <div class="card2">
    ...
    methods: {
      async submitForm() {
        try {
          ...
          console.log("token recu apres inscription : ", response.data.token)
          localStorage.setItem('token', response.data.token);
  
          alert('Compte créé avec succès ! Vous pouvez maintenant vous connecter.');
          this.$router.push('/login'); // Redirige vers la connexion après inscription
        } catch (error) {
  ...
  <style>
      ...
      .card {
        background-image: linear-gradient(163deg, #d900ff 0%, #3700ff 100%);
        border-radius: 22px;
        transition: all 0.3s;
      }
